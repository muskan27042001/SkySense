...
class ForecastAdapterNewActivity : RecyclerView.Adapter<ForeCastNewActivityHolder>() {

    private var listofforecast = listOf<WeatherList>()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ForeCastNewActivityHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.fourdayforecast, parent, false)
        return ForeCastNewActivityHolder(view)
    }
    ...
    override fun onBindViewHolder(holder: ForeCastNewActivityHolder, position: Int) {
        val forecastObject = listofforecast[position]
        for (i in forecastObject.weather){
            holder.description.text = i.description!!
        }

        val maxtemperatureFahrenheit =forecastObject.main?.tempMax.toString()
        val maxtemperatureCelsius = (maxtemperatureFahrenheit.toDouble()?.minus(273.15))
        val maxtemperatureFormatted = String.format("%.2f", maxtemperatureCelsius)

        val mintemperatureFahrenheit = forecastObject.main?.tempMin.toString()
        val mintemperatureCelsius = (mintemperatureFahrenheit.toDouble()?.minus(273.15))
        val mintemperatureFormatted = String.format("%.2f", mintemperatureCelsius)
...
class ForeCastNewActivityHolder(itemView: View) : RecyclerView.ViewHolder(itemView){
